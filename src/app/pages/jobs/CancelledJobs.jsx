...
const CancelledJobs = () => {
  ...
  const getJobs = async () => {
    setLoader(true)
    try {
      const response = await ApiGet(`job?status=cancelled`)
  ...
  return (
    <>
      <PageTitle breadcrumbs={CancelledBreadCrumbs}>
        {intl.formatMessage({id: 'MENU.JOBS.CANCELLED_JOBS'})}
      </PageTitle>
      <DataTable
